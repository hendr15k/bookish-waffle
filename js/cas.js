...
class CAS {
    constructor() {
        this.variables = {
            'pi': new Num(Math.PI), // Store numeric value? Or should we use SymbolNode?
            // If we store Num, implicit N() works, but symbolic calc loses precision.
            // Let's store SymbolNode by default in a separate map or just rely on 'pi' symbol.
        ...
        // Pre-defined constants as SymbolNodes in variables?
        // No, let them be resolved by SymbolNode.evaluateNumeric() or if user assigns them.
        ...
        if (node instanceof Call) {
            ...
            if (this.functions.hasOwnProperty(node.funcName)) {
                ...
                for (let i = 0; i < funcDef.params.length; i++) {
                    ...
                    const argVal = args[i];
                    body = body.substitute(new SymbolNode(paramName), argVal);
                }
            ...
            if (node.funcName === 'diff') {
                ...
                if (!(varNode instanceof SymbolNode)) throw new Error("Second argument to diff must be a variable");
            ...
            if (node.funcName === 'integrate') {
                ...
                const func = args[0];
                const varNode = args[1];
                if (!(varNode instanceof SymbolNode)) throw new Error("Second argument to integrate must be a variable");
            ...
            if (node.funcName === 'solve') {
                 ...
                 const eq = args[0];
                 const varNode = args[1];
                 if (!(varNode instanceof SymbolNode)) throw new Error("Second argument to solve must be a variable");
            ...
            if (node.funcName === 'plot') {
                 ...
                 const expr = args[0];
                 const varNode = args[1];
                 const min = args.length > 2 ? args[2].evaluateNumeric() : -10;
                 const max = args.length > 3 ? args[3].evaluateNumeric() : 10;

                 if (isNaN(min) || isNaN(max)) throw new Error("Plot range must be numeric");
            ...
            if (node.funcName === 'taylor') {
                ...
                if (!(varNode instanceof SymbolNode)) throw new Error("Second argument to taylor must be a variable");
            ...
            if (node.funcName === 'limit') {
                ...
                const varNode = args[1];
                const point = args[2];
                if (!(varNode instanceof SymbolNode)) throw new Error("Second argument to limit must be a variable");
...
N(expr) [numeric eval], clear(), help()`;
    ...
    _sum(expr, varNode, start, end) {
        if (!(varNode instanceof SymbolNode)) throw new Error("Sum variable must be a symbol");
        if (!(start instanceof Num) || !(end instanceof Num)) {
            // SymbolNodeic sum not implemented, return call
    ...
    _product(expr, varNode, start, end) {
        if (!(varNode instanceof SymbolNode)) throw new Error("Product variable must be a symbol");
        if (!(start instanceof Num) || !(end instanceof Num)) {
    ...
    _solve(eq, varNode) {
        ...
        try {
            ...
            if (diffQuad instanceof Num && diffQuad.value === 0 && !(A instanceof Num && A.value === 0)) {
    ...
    _limit(expr, varNode, point, depth = 0) {
        ...
        if (expr instanceof Div) {
            ...
            if (num instanceof Num && den instanceof Num) {
                ...
                if (num.value !== 0 && den.value === 0) return new SymbolNode("Infinity");
