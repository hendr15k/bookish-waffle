...
class CAS {
    ...
    _recursiveEval(node) {
        ...
        if (node instanceof Call) {
            ...
            if (node.funcName === 'odesolve') {
                // odesolve(eq, depVar, indepVar, initCond, endVal, step)
                if (args.length < 5) throw new Error("odesolve requires at least 5 arguments: eq, depVar, indepVar, init, end");
                return this._odesolve(args[0], args[1], args[2], args[3], args[4], args.length > 5 ? args[5] : new Num(0.1));
            }

            if (node.funcName === 'slopefield') {
                if (args.length < 3) throw new Error("slopefield requires 3 arguments: eq, x, y");
                const expr = args[0];
                const xVar = args[1];
                const yVar = args[2];
                // normalize eq y' = f(x, y)
                // Assuming expr is f(x, y) if simple expression, or solve for diff if equation
                // For simplicity, user provides f(x,y)
                // We return vector (1, f(x,y))
                return {
                    type: 'plot',
                    subtype: 'slopefield',
                    expr: expr,
                    varX: xVar,
                    varY: yVar,
                    min: -10, max: 10, // Defaults, can be overridden by plot bounds
                    toString: () => `Slope Field of ${expr}`,
                    toLatex: () => `\\text{Slope Field } y' = ${expr.toLatex()}`
                };
            }

            if (node.funcName === 'vectorfield') {
                if (args.length < 3) throw new Error("vectorfield requires 3 arguments: [u, v], x, y");
                const vec = args[0];
                const xVar = args[1];
                const yVar = args[2];
                if (!(vec instanceof Vec) || vec.elements.length !== 2) throw new Error("First argument must be a vector of 2 components");

                return {
                    type: 'plot',
                    subtype: 'vectorfield',
                    xExpr: vec.elements[0],
                    yExpr: vec.elements[1],
                    varX: xVar,
                    varY: yVar,
                    min: -10, max: 10,
...
and, or, not, xor, int, evalf`;
    ...
    _odesolve(eq, depVar, indepVar, initCond, endVal, step) {
        // eq: y' = f(x, y) or expression f(x,y)
        // initCond: [x0, y0]
        ...
        if (!(initCond instanceof Vec) || initCond.elements.length !== 2) throw new Error("Initial condition must be [x0, y0]");
        const x0 = initCond.elements[0].evaluateNumeric();
        const y0 = initCond.elements[1].evaluateNumeric();
        const xEnd = endVal.evaluateNumeric();
        const h = step.evaluateNumeric();

        if (isNaN(x0) || isNaN(y0) || isNaN(xEnd) || isNaN(h)) throw new Error("Invalid numeric inputs for odesolve");

        // Prepare function f(x, y)
        // We substitute indepVar -> x, depVar -> y
        // But evaluateNumeric needs numbers.
        // We can use substitute, but it's slow.
        // Pre-simplification?

        const f = (valX, valY) => {
            try {
                // substitute x and y
                // Optimization: if func is simple, maybe structure it?
                // For now, raw substitution
                let res = func.substitute(indepVar, new Num(valX));
                res = res.substitute(depVar, new Num(valY));
                return res.simplify().evaluateNumeric();
            } catch(e) { return NaN; }
        };
        ...
        for(let i=0; i<steps; i++) {
            ...
            const k2 = f(currX + h*0.5*sign, currY + h*0.5*k1*sign);
            const k3 = f(currX + h*0.5*sign, currY + h*0.5*k2*sign);
            const k4 = f(currX + h*sign, currY + h*k3*sign);

            currY = currY + (h/6) * (k1 + 2*k2 + 2*k3 + k4) * sign;
            currX = currX + h*sign;
        ...
        return {
            ...
            subtype: 'list',
            scatter: points,
            toString: () => `ODE Solution (${points.length} points)`,
            toLatex: () => `\\text{ODE Solution}`
        };
    }
