...
class Div extends BinaryOp {
    ...
    simplify() {
        ...
        // Infinity handling
        if (r instanceof Sym && r.name === 'Infinity') {
            if (l instanceof Num || (l instanceof Sym && l.name !== 'Infinity')) return new Num(0);
        }
...
class Pow extends BinaryOp {
    ...
    simplify() {
        ...
        if (l instanceof Vec && r instanceof Num && Number.isInteger(r.value) && r.value >= 0) {
            if (r.value === 0) {
                // Identity matrix? Need size.
                // Assuming square matrix.
                if (l.elements.length > 0 && l.elements[0] instanceof Vec) {
            ...
            while (exp > 0) {
                if (exp % 2 === 1) {
                    result = new Mul(result, base).simplify();
                }
