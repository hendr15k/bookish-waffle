...
class SymbolNode extends Expr {
    ...
    equals(other) { return other instanceof SymbolNode && this.name === other.name; }
}
...
class Mul extends BinaryOp {
    ...
    simplify() {
        ...
        if (l instanceof SymbolNode && l.name === 'i' && r instanceof SymbolNode && r.name === 'i') {
            return new Num(-1);
        }
...
class Div extends BinaryOp {
    ...
    integrate(varName) {
        if (this.left instanceof Num && this.left.value === 1 && this.right instanceof SymbolNode && this.right.name === varName.name) {
...
class Pow extends BinaryOp {
    ...
    simplify() {
        ...
        // i^2 = -1
        if (l instanceof SymbolNode && l.name === 'i' && r instanceof Num && r.value === 2) {
            return new Num(-1);
        }
    ...
    integrate(varName) {
        if (this.left instanceof SymbolNode && this.left.name === varName.name) {
            if (this.right instanceof Num) {
...
class Call extends Expr {
    ...
    simplify() {
        ...
        if (this.funcName === 'sqrt') {
            ...
            if (arg instanceof Num) {
                ...
                if (Number.isInteger(sqrtVal)) {
                    return new Mul(new SymbolNode('i'), new Num(sqrtVal));
                }
                return new Mul(new SymbolNode('i'), new Call('sqrt', [new Num(absVal)]));
            }
            // sqrt(x) -> x^0.5
        ...
        if (this.funcName === 'ln') {
            ...
            if (arg instanceof SymbolNode && arg.name === 'e') return new Num(1);
        }
