...
// --- Calculus ---
    ...
    "rk4": { "description": "Runge-Kutta 4th order ODE solver (fixed step).", "syntax": "rk4(f, y, t, y0, t0, t1, h)", "examples": ["rk4(y - t*t + 1, y, t, 0.5, 0, 2, 0.2)", "rk4([y2, -k*y1], [y1, y2], t, [1, 0], 0, 20, 0.1)"] },
    "rk45": { "description": "Adaptive Dormand-Prince RK4(5) solver with step-size control and error estimates. Returns Vec rows [t, y, err] plus metadata.", "syntax": "rk45(f, y, t, y0, t0, t1, h0, tol, opts)", "examples": ["rk45(-0.5*y, y, t, 1, 0, 2, 0.1, 1e-6, [])", "rk45([y2, -y1], [y1,y2], t, [0,1], 0, 10, 0.1, 1e-6, [])", "rk45([y2, -c*y2 - k*y1], [y1, y2], t, [1, 0], 0, 20, 0.1, 1e-6, [])"] },
    "bs45": { "description": "Adaptive Bogacki-Shampine 4(5) solver — FSAL (first-same-as-last) pair. Reuses 3 of 6 stages from the previous step, making it more efficient than Dormand-Prince. Step accepted when error <= tol.", "syntax": "bs45(f, y, t, y0, t0, t1, h0, tol, opts)", "examples": ["bs45(-0.5*y, y, t, 1, 0, 2, 0.1, 1e-6, [])", "bs45([y2, -y1], [y1,y2], t, [0,1], 0, 10, 0.1, 1e-6, [])", "bs45([y2, -0.2*y2 - y1], [y1, y2], t, [1, 0], 0, 20, 0.1, 1e-6, [])"] },
    "odestats": { "description": "Extracts and formats metadata from an adaptive ODE solver result (rk45, bs45). Returns a Vec of labeled statistics: steps, accepted, rejected, fEvals, avgStep, finalT, and finalY values.", "syntax": "odestats(sol)", "examples": ["odestats(rk45(-0.5*y, y, t, 1, 0, 2, 0.1, 1e-6, []))", "odestats(bs45(-0.5*y, y, t, 1, 0, 2, 0.1, 1e-6, []))", "odestats(bs45([y2, -0.2*y2 - y1], [y1, y2], t, [1, 0], 0, 10, 0.1, 1e-6, []))", "odestats(rk45([y2, -y1], [y1, y2], t, [0, 1], 0, 20, 0.1, 1e-6, []))"] },
    "odeplot": { "description": "Generates a plot from the solution table returned by rk4 or rk45.", "syntax": "odeplot(sol)", "examples": ["odeplot(rk45(-0.5*y, y, t, 1, 0, 2, 0.1, 1e-6, []))", "odeplot(rk4([y2, -y1], [y1, y2], t, [1, 0], 0, 10, 0.1))"] },
...
// --- Utils ---
    ...
    "invmod": { "description": "Alias for modInverse.", "syntax": "invmod(a, m)", "examples": ["invmod(3, 11)"] },
    "inverse": { "description": "Alias for modInverse.", "syntax": "inverse(a, m)", "examples": ["inverse(3, 11)"] },
    "modpow": { "description": "Alias for modPow.", "syntax": "modpow(base, exp, m)", "examples": ["modpow(2, 10, 1000)"] },
    "powmod": { "description": "Alias for modPow.", "syntax": "powmod(base, exp, m)", "examples": ["powmod(2, 10, 1000)"] },
    "mod_pow": { "description": "Alias for modPow.", "syntax": "mod_pow(base, exp, m)", "examples": ["mod_pow(2, 10, 1000)"] },
    "extendedGcd": { "description": "Alias for xgcd.", "syntax": "extendedGcd(a, b)", "examples": ["extendedGcd(240, 46)"] },
    "extended_gcd": { "description": "Alias for xgcd.", "syntax": "extended_gcd(a, b)", "examples": ["extended_gcd(240, 46)"] },
    "egcd": { "description": "Alias for xgcd.", "syntax": "egcd(a, b)", "examples": ["egcd(240, 46)"] },
