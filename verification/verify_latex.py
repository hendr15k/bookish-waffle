...
def run():
    ...
    with sync_playwright() as p:
        ...
        # Take screenshot
        screenshot_path = "verification/latex_fix.png"
        page.screenshot(path=screenshot_path, full_page=True)
        print(f"Screenshot saved to {screenshot_path}")

        # We can also check the innerHTML of the results to verify LaTeX strings are present
        # even if MathJax fails to render (because no internet in sandbox)
        content = page.content()
        if "\\arcsin\\left(x\\right)" in content:
            print("Verified: \\arcsin present in DOM")
        else:
            print("FAILED: \\arcsin NOT found in DOM")

        if "\\left(A\\right)^{-1}" in content:
             print("Verified: inv present in DOM")
        else:
             print("FAILED: inv NOT found in DOM")
